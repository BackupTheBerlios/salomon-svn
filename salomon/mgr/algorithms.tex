...
Utworzone w~ten sposób drzewo u¿ywane jest do póŸniejszego klasyfikowania
...
\begin{enumerate}
  ...
  (nie decyzyjnych) i~tworzony jest wêze³ decyzyjny.
...
Spoœród wszystkich atrybutów nie decyzyjnych w~ka¿dym przebiegu algorytmu
...
najbardziej przydany. Przypuœæmy, ¿e atrybut decyzyjny $D$ przyjmuje wartoœci
...
Wyra¿enie to~okreœla entropiê wzglêdem atrybutu decyzyjnego D dla konkretnej
wartoœci atrybutu $a_j$, gdzie $p(\frac{d_i}{a_j})$ oznacza prawdopodobieñstwo, ¿e zachodzi $d_i$ pod warunkiem $a_j$ 

Œrednia entropia atrybutu $A$ wzglêdem atrybutu decyzyjnego $D$ wyra¿ona jest 
wzorem:
...
atrybutów oraz z~niepe³nymi danymi (rekordami wype³nionymi tylko czêœciowo),
...
Kolejnym ograniczeniem algorytmu \emph{ID3} jest mo¿liwoœæ obs³ugi tylko
...
\begin{itemize}
  ...
  wartoœæ atrybutu $A$ jest mniejsza b¹dŸ równa $a_i$ i~wiêksza od $a_i$
...
poziomu b³êdów dla rzeczywistych danych.
...
Dziêki przycinaniu uzyskuje siê wiêksz¹ generalizacjê oceny nowych przypadków.
