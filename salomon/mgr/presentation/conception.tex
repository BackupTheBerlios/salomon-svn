\section{Za³o¿enia i koncepcja platformy}

\frame{
	\frametitle{Analiza wymagañ}
	\begin{itemize}		
		\item Przyjazny interfejs u¿ytkownika
		\item Dobrze zdefiniowany interfejs programistyczny
		\item Odizolowanie algorytmów od szczegó³ów implementacyjnych platformy
		\item Spójna reprezentacja wiedzy dla wszystkich algorytmów
		\item Komponentowa architektura
		\item Wykorzystanie gotowych implementacji algorytmów
		\item Otwarta licencja \emph{(LGPL)}
	\end{itemize}		
}

\frame{
	\frametitle{G³ówne za³o¿enia}
	\begin{itemize}
			\item Koncepcja zadaniowoœci			
			\begin{itemize}
				\item Zadanie -- atomowa jednostka reprezentuj¹ca obliczenia
				\item Tworzenie powi¹zañ miêdzy zadaniami
			\end{itemize}
    	\item Budowa komponentowa
    	\item Otwartoœæ architektury
    	\item Niezale¿noœæ od œrodowiska wdro¿enia
    	\item Prostota u¿ytkowania
    \end{itemize}
}

