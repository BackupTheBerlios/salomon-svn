Przedstawione w~poprzednim rozdziale narzêdzia wspomagaj¹ce prowadzenie obliczeñ zwi¹zanych z~zagadnieniami uczenia maszynowego, jak choæby Weka czy Vinlen,  posiadaj¹ ograniczenia, które utrudniaj¹ ich wykorzystanie do pewnych zastosowañ.
Zaproponowana poni¿ej koncepcja zak³ada stworzenie przyjaznego œrodowiska pozwalaj¹cego na odkrywanie i~przechowywanie wiedzy
oraz uruchamianie ró¿nych algorytmów \emph{machine learning}. Jest ona rezultatem analizy wymagañ, stanowi¹cych odpowiedŸ na ograniczenia \emph{Weki} i~\emph{Vinlena}.
...
%-gui
Jednym z~podstawowych za³o¿eñ stawianych aplikacj¹ wspomagaj¹cym proces uczenia maszynowego jest prosty i~przyjazny \textbf{interfejs u¿ytkownika}.
Czêsto cech¹ decyduj¹c¹ o~rozpoczêciu u¿ywania danej aplikacji jest pierwszy kontakt z~ni¹.
...
jak i~osoby wykonuj¹cej eksperymenty z~wykorzystaniem gotowych wtyczek. Z~kolei
dostarczenie przyk³adowego zestawu wtyczek i~danych, pozwoli
na ³atwe u¿ycie i~przetestowanie ka¿dego z~aspektów platformy.

%-api
Oprócz przyjaznego graficznego interfejsu u¿ytkownika istotne jest równie¿
dostarczenie prostego i~dobrze zdefiniowanego \textbf{interfejsu programistycznego}
...
aplikacji, czyli np. przechowywania i~dostêpu do danych czy kwestii komunikacji
...
i natychmiastowego przetestowania zachowania algorytmu bez koniecznoœci
...
odpowiedni poziom bezpieczeñstwa. Dostarczenie API 
pozwalaj¹cego na tworzenie, manipulacje, odczyt i~zapis drzew decyzyjnych, umo¿liwi umieszczenie algorytmów odpowiadaj¹cych ró¿nym aspektom pracy na drzewach decyzyjnych w~osobnych wtyczkach, niekoniecznie dostarczanych przez tego samego dostawce. 
Jednak przede wszystkim praca nad jedn¹ czêœci¹ (np. prezentacj¹ drzew) nie wymaga³oby 
wiedzy na temat innych wtyczek.

% - wiedza
\label{lab:knowledge}
System wspomagaj¹cy procesy uczenia maszynowego,
...
W zaprezentowanych w~poprzednim rozdziale systemach u¿ywane s¹ jednowymiarowe zbiory trenuj¹ce (jedna tabela, bez powi¹zañ z~innymi). Taki model pasuje do
wiêkszoœci algorytmów ucz¹cych, jednak 
istnieje pewne ryzyko utraty danych, które nios¹ z~sob¹ relacje miêdzy danymi,
dlatego wa¿na jest mo¿liwoœæ operowania na wielowymiarowych zbiorach 
...
W dostêpnych na rynku aplikacjach, przyk³adowo w~Wece, zbiór danych jest 
podzbiorem zbioru atrybutów, co powoduje dodatkowe kopiowanie danych
z~zewnêtrznego Ÿród³a. W~celu unikniêcia tego dodatkowego narzutu zbiór danych, w~postaci zapytania stanowi¹cego zbiór warunków,
powinien byæ utworzony przed zbiorem atrybutów.
Zatem zbiór atrybutów stanowiæ bêdzie opis transformacji oryginalnych danych
na dane wejœciowe do algorytmów ucz¹cych. Wyró¿nienie warstwy atrybutów jest konieczne,
gdy¿ wiêkszoœæ algorytmów wymaga na wejœciu przetworzonych danych.
...
Powy¿sze rozwi¹zanie zapewnia szereg korzyœci. Atrybuty budowane na zbiorach danych umo¿liwi¹
...
Poniewa¿ zbiory danych stanowi¹ jedynie opis, w~jakiœ sposób oryginalne dane mog¹ zostaæ wydobyte
...
dostawców. Dziêki odseparowaniu wtyczek od platformy i~przeniesieniu na ni¹ 
...
Przeniesienie ciê¿aru obliczeñ z~platformy do wtyczek poci¹ga za sob¹ koniecznoœæ
zapewnienia przez platformê mechanizmów aktualizacji wtyczek. Mog¹ byæ one umieszczane
w zdalnych lokalizacjach, tak wiêc system powinien posiadaæ mechanizmy kontroluj¹ce
...
umo¿liwi \textbf{wykorzystanie gotowych algorytmów}. W~systemie \emph{Vinlen} zosta³y czêœciowo 
wykorzystane implementacje algorytmów przygotowanych do wczeœniejszych 
projektów jak np. program \emph{Emerald} (przypisek). Znajdziemy tam takie algorytmy jak 
\emph{Abacuse}, \emph{AQ} czy \emph{Cluster}. Czêœæ z~tych implementacji jest bardzo nowatorska i~dysponuje 
...
% - agenty
Systemy pracuj¹ce w~rozproszonym œrodowisku powinny reagowaæ na zmiany w~nim zachodz¹ce.
...
obliczeñ. Wymaganie to~realizowaæ bêd¹ tzw. \textbf{agenty}.
...
W dobie rozpowszechniania siê technologii gridowych i~klastrowych aplikacje powinny
...
wymieniaæ jak najmniejsz¹ iloœæ informacji, by nie obci¹¿aæ nadmiernie zasobów komunikacyjnych.

Istotna wydaje siê tak¿e potrzeba zapewnienia mo¿liwoœci \textbf{zrównoleglenia} niektórych obliczeñ.
...
pozwalaj¹ce na zrównoleglenie aplikacji wielow¹tkowej w~sposób przeŸroczysty 
