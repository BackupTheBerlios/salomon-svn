...
Przedstawiona w poprzednim rozdziale koncepcja systemu pozwala na elastyczne rozszerzanie jego mo¿liwoœci dziêki zastosowaniu architektury komponentowej.
Poni¿ej przedstawiono elementy adekwatnej implementacji. Jej  g³ówn¹ czêœæ stanowi platforma, której mo¿liwoœci mog¹ byæ rozszerzane za pomoc¹ wtyczek.

\section{Platforma}
\label{lab:platform-interfaces}

Trzonem systemu jest platforma, która
...
zadania. Odpowiada tak¿e za komunikacje miêdzy innymi instancjami \emph{Salomona}.
...
Interfejs \emph{IPluginManager} zarz¹dza wtyczkami (\ref{lab:plugin}). Pozwala na zapisanie informacji o~nowej wtyczce
...
\emph{ISolutionManager} zarz¹dza obiektami implementuj¹cymi interfejs \emph{ISolution} \ref{lab:solution}.

\paragraph{IProjectManager}

Interfejs ten umo¿liwia zarz¹dzanie projektami zdefiniowanymi dla
okreœlonego obiektu \emph{ISolution} (\ref{lab:project}) .

\paragraph{ITaskManager}

Interfejs ten dostarcza funkcjonalnoœci umo¿liwiaj¹cej zarz¹dzanie zadaniami (\ref{lab:tasks}) dla konkretnego projektu. Oprócz standardowych operacji wspólnych dla wszystkich menad¿erów, jego zadaniem jest tak¿e zapewnienie poprawnej konfiguracji, nadzór nad wykonaniem oraz zachowanie informacji
o przebiegu przetwarzania zadañ.
...
drzewach decyzyjnych odpowiednio za pomoc¹ interfejsów \emph{IDataSetManager}, \emph{IAttributeManager} i \emph{ITreeManager}.
