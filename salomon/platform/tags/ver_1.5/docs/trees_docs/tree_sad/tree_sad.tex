...
\begin{tabular}{|l|l|l|l|}
  ...
  08-11-2005 & 1.2 & Opis pluginu wnioskuj¹cego & Mateusz Nowakowski\\
             &     & Aktualizacja wiêkszoœci rozdzia³ów &            \\
  \hline
  15-11-2005 & 1.3 & Poprawa rozdzia³u 3. Przypadki   & Mateusz Nowakowski\\
             &     & u¿ycia &            \\
...
\begin{figure}[ht]
		\center
    \includegraphics[width=0.40\textheight]{sad_files/use_case.png}
    \caption{Diagram przypadków u¿ycia}
    \label{fig:use_case_view}
\end{figure}

Aby zdefiniowaæ jakiekolwiek drzewo decyzyjne u¿ytkownik musi okreœliæ na jakich danych pragnie je stworzyæ. Tworzenie drzewa polega na wyborze algorytmu buduj¹cego oraz odpowiednie skonfigurowanie jego parametrów. U¿ytkownik musi równie¿ mieæ mo¿liwoœæ zarz¹dzania istniej¹cymi drzewami oraz móc je odpowiednio wizualizowaæ. Drzewa decyzyjne tworzy siê na pewnym ma³ym zbiorze danych by nastêpnie zweryfikowaæ ich poprawnoœæ na wiekszym zbiorze danych. U¿ytkownik zatem powinien mieæ mo¿liwoœc wykorzystania drzew w praktyce.

Powy¿sze przypadki u¿ycia oraz za³o¿enia platfomy Salomon prowadza do nastêpuj¹cych operacji u¿ytkownika w ramach platformy.
...
wybiera tabelê a nastêpnie kolumnê reprezentuj¹c¹ wynik decyzji --  \textit{kolumna decyzyjna} oraz listê kolumn reprezentuj¹ce przes³anki decyzji -- \textit{kolumny decyduj¹ce}. Nastêpnie musi okreœliæ, na których wierszach bêdzie budowane drzewo decyzyjne. U¿ytkownik mo¿e wybraæ wiêcej ni¿ jedno Ÿród³o danych, z których potem bêd¹ generowane drzewa decyzyjne. 
...
Ka¿de drzewo decyzyjne mo¿na zweryfikowaæ. Samodzielny plugin wnioskuj¹cy umo¿liwia przeprowadzenie wnioskowania za pomoc¹ wybranego drzewa na innym zbiorze danych. U¿ytkownik wybiera drzewo oraz zbiór danych, nastêpniê w oknie wyników przedstawione s¹ wnioski np. procent poprawnoœci, które wiersze zawieraj¹ inny wynik ni¿ spodziewany itp.
...
\item Zestawu 4 typów pluginów: pluginu definiuj¹cego dane wejœciowe, pluginu algorytmicznego, pluginu wizualizuj¹cego, zarz¹dzaj¹cego drzewami oraz pluginu wnioskuj¹cego
\end{itemize}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.55\textheight]{sad_files/tree_interfaces.png}
...
Jak wczeœniej wspomniano bêd¹ 4 rodzaje pluginów:
\begin{itemize}
...
Plugin wnioskuj¹cy mo¿e równie¿ pracowaæ w dwóch trybach. Jednak zalecana jest praca samodzielna. W przypadku gdy w œrodowisku znajdujê siê lista identyfikatorów drzew wówczas plugin uruchamia wnioskowanie dla ka¿dego drzewa z tej listy. Danymi wejœciowymi s¹ tabele, z których drzewa powsta³y pomijaj¹c wiersze, z których przeprowadzane by³o budowanie drzewa. Plugin w oknie rezultatów przedstawia wnioski z testów dla ka¿dego drzewa w postaci ogólnych informacji o drzewie (jego nazwa, zbiór danych, z którego powsta³), informacja na jakich wierszach by³o przeprowadzone testowanie oraz wynik testów:
\begin{itemize}
\item Procent zgodnoœci - stosunek liczby wierszy z poprawnym wynikiem testu do ogólnej liczby testowanych wierszy
\item Lista wierszy (przedzia³ów) dla których wynik testu by³ nieprawid³owy
\end{itemize}

W przypadku pracy samodzielnej (tzn. w sytuacji kiedy plugin nie wykryje obecnoœci listy drzew w œrodowisku) u¿ytkownik wybiera drzewo, które chce testowaæ, oraz okreœla zbiór danych do testowania. Zbiór danych musi nale¿eæ do tej samej tabeli z której drzewo powsta³o, ale musi zawieraæ inne wiersze. Wynik wnioskowania jest identyczny jak w przypadku pracy niesamodzielnej.
...
Baza wejœciowa musi zawieraæ tabelki lub widoki zawieraj¹ce dane. W celu okreœlenia danych wejœciowych u¿ytkownik musi wybraæ tabelê a nastêpnie kolumnê reprezentuj¹c¹ wynik decyzji, nazwijmy j¹ kolumn¹ decyzyjn¹ oraz listê kolumn reprezentuj¹ce przes³anki decyzji, nazwijmy je kolumnami decyduj¹cymi. Nastêpnie u¿ytkownik definiujê podzbiór tabeli lub widoku, na których bêdzie budowane drzewo. Innymi s³owy format danych wejœciowych jest zwyk³¹ tabelk¹. Wybrana tabela i kolumny wraz z definicj¹ bazy danych zostaje zapisana w bazie definicyjnej do póŸniejszego u¿ycia przez pluginy algorytmiczne.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textheight]{sad_files/erd.png}
    \caption{Diagram ERD}
    \label{fig:erd}
\end{figure}
...
\item TREE\_DATA\_SOURCES - tabela reprezentuj¹ce wybrane przez u¿ytkownika dane wejœciowe potrzebne do zbudowania drzewa. Zawiera m.in.: wskaŸnik do rozwi¹zania (solution) oraz nazwy wybranej wraz z wybranymi kolumnami: kolumnê decyzyjnej i kolumny decyduj¹ce, numer pierwszego i ostatniego wiersza. Informacja o rozwi¹zaniu potrzebna jest w celu zidentyfikowania parametrów bazy wejœciowej oraz w celu umo¿liwienia korzystania z danych wejœciowych przez wszystkie projekty w rozwi¹zaniu.
