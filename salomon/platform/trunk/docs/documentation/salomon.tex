%& --translate-file=cp1250pl
%bez powyzszej linijki nie dzialaja polskie znaki (musi ona byc w pierwszej linii dokumentu
%%&platex --translate-file=il2-pl % przeyestowac z tym
\documentclass[a4paper,12pt]{article}
\usepackage[polish]{babel}
\usepackage[T1]{fontenc}
\usepackage[dvips]{graphicx}
\usepackage{fancyvrb}

\usepackage[urlcolor=blue, colorlinks=true, bookmarks=true, bookmarksnumbered=true]{hyperref}
%\usepackage{intendfirst}
\usepackage{times}
\usepackage{epsfig}
\usepackage{color}

\usepackage{fancyhdr} 

\pagestyle{fancy} 

\rfoot{SALOMON}


%ustawienie wielkosci akapitu
%\setlength{\parindent}{0mm}
%%odstep miedzy akapitami
%\setlength{\parskip}{2mm}

%ustawienia rozmiaru tekstu
    \textwidth 16cm
    \textheight 24cm 
    \topmargin -15mm
    \evensidemargin-3mm
    \oddsidemargin -3mm
    


\title{Salomon}

\author{Nikodem Jura \emph{(\href{nico@icslab.agh.edu.pl}{nico@icslab.agh.edu.pl})}
        \\
        Krzysztof Rajda \emph{(\href{krzycho@student.uci.agh.edu.pl}{krzycho@student.uci.agh.edu.pl})}
        \\
        Jakub Ga³kowski \emph{(\href{avi@student.uci.agh.edu.pl}{avi@student.uci.agh.edu.pl})}
        \\
        Jakub Paw³owski \emph{(\href{qoooba@student.uci.agh.edu.pl}{qoooba@student.uci.agh.edu.pl})}}               
        

\begin{document}

\maketitle
%wylaczenie numerowania na I stronie
\thispagestyle{empty}

\begin{center}
\textcolor{blue}{\href{http://salomon.iisg.agh.edu.pl}{http://salomon.iisg.agh.edu.pl}}
\end{center}

\begin{center}
Prowadz¹cy: dr in¿. Marek Kisiel-Dorohinicki
\end{center}

\newpage

%spis tresci
\tableofcontents
\newpage

%reszta
\include{intro}
\include{concepts}
\include{functionality}
\include{structure}
\include{remote}
\include{action}
\include{db_desc}
\include{user_plugin}
\include{running}
\include{ui}
\include{production}
%\include{junit}
%\include{abbot}
%\include{grinder}
\include{security}
\include{roadmap}
\include{links}

\end{document}
