...
public final class GraphTaskManagerGUI {
	private static final Logger LOGGER = Logger
			.getLogger(GraphTaskManagerGUI.class);
		...
		try {
			...
			Utils
					.showErrorMessage(Messages
							.getString("ERR_CANNOT_LOAD_PLUGIN"));
		}
	...
	public void showResultPanel(ITask task) {
		...
		try {
			...
			Utils.showErrorMessage(Messages
					.getString("ERR_CANNOT_SHOW_TASK_RESULT"));
			return;
		}
		...
		JOptionPane.showMessageDialog(_parent, comp, Messages
	...
	public void showSettingsPanel(ITask task) {
		...
		try {
			...
			LOGGER.fatal("", e1);
			Utils.showErrorMessage(Messages
					.getString("ERR_CANNOT_SHOW_TASK_SETTINGS"));
			return;
		}
		// if settings are null, default settings are taken
		if (inputSettings == null) {
			inputSettings = plugin.getSettingComponent(_platformUtil)
					.getDefaultSettings();
		}
		...
		// showing dialog
		SettingsDialog settingsDialog = new SettingsDialog(_parent, Messages
				.getString("TIT_PLUGIN_SETTINGS"));
		settingsDialog.setSeparator("Plugin settings");
		settingsDialog.setSettingsComponent(taskSettingsComponent);
		ValidationResultModel resultModel = _platformUtil
	...
	public void startTasks() {
		List<ITask> executionPlan = GraphPlanner.makePlan(_taskGraphEditor
				.getGraph());
	...
	public void viewTasks() {
		if (_tasksViewerFrame == null) {
			...
			// FIXME:
			_tasksViewerFrame.getContentPane().add(new TaskSpread(null));
			_tasksViewerFrame.pack();
		}
		_tasksViewerFrame.setLocation(Utils
				.getCenterLocation(_tasksViewerFrame));
	...
	private boolean setTaskProperties(ITask iTask) throws PlatformException {
		...
		String cdate = dcdate == null ? "" : DateFormat.getDateInstance()
				.format(dcdate)
				+ " " + DateFormat.getTimeInstance().format(dcdate);
		String mdate = dmdate == null ? "" : DateFormat.getDateInstance()
				.format(dmdate)
