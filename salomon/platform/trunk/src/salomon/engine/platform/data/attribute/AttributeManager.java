...
public final class AttributeManager implements IAttributeManager {
	...
	public void add(IAttributeSet attributeSet) throws PlatformException {
		...
		insertAttrSet.addValue("ATTRIBUTESET_CD", new Date(System
				.currentTimeMillis()));
			...
			try {
				...
				if (iD instanceof IEnumAttributeDescription) {
					...
					for (Object ob : iE.getPossibleValues()) {
						...
						insertAllowedValues
								.setTable(AttributeManager.ATTRIBUTE_SET_ITEM_ENUM_ALLOWED_VALUES);
	...
	private void proceedWithLateQueries() throws PlatformException {
		...
		while (en.hasMoreElements()) {
			IEnumAttributeDescription ie = (IEnumAttributeDescription) en
					.nextElement();
			for (Object ob : getAllowedValuesForAttrSetItem(_invokeLaterEnumTypes
					.get(ie))) {
	...
	private IAttributeDescription retrieveAttrDescFromDb(ResultSet resultSet)
		...
		case ATTRIBUTE_TYPE_INTEGER:
			return this.createIntegerAttributeDescription(resultSet
					.getString("COLUMN_NAME"));
		case ATTRIBUTE_TYPE_REAL:
			return this.createRealAttributeDescription(resultSet
					.getString("COLUMN_NAME"));
		case ATTRIBUTE_TYPE_ENUM:
			...
			_invokeLaterEnumTypes.put(ie, resultSet.getInt("ATTRIBUTESET_ITEM_ID"));
			return ie;
		case ATTRIBUTE_TYPE_STRING:
			return this.createStringAttributeDescription(resultSet
					.getString("COLUMN_NAME"));
		case ATTRIBUTE_TYPE_DATE:
			return this.createDateAttributeDescription(resultSet
					.getString("COLUMN_NAME"));
		default:
		...
		try {
			...
			while (resultSet.next()) {
				...
				if (attrSetId != attrSetTmpId) {
					...
					IAttributeDescription[] iTable = new IAttributeDescription[tmpDescriptions
							.size()];
					for (int i = 0; i < tmpDescriptions.size(); i++) {
						iTable[i] = tmpDescriptions.elementAt(i);
					}
					IAttributeSet iAttributeSet = this
							.createAttributeSet(iTable);
					iAttributeSet.setName(resultSet
							.getString("ATTRIBUTESET_NAME"));
					iAttributeSet.setInfo(resultSet
							.getString("ATTRIBUTESET_INFO"));
	...
	public IAttributeSet getAttributeSet(IUniqueId id) throws PlatformException {
		...
		select.addColumn("di.COLUMN_TYPE");
		select.addCondition("d.ATTRIBUTESET_ID = di.ATTRIBUTESET_ID");
		select.addCondition("d.ATTRIBUTESET_ID = " + id.getId());
		...
		try {
			...
			if (tmpDescriptions.size() > 0) {
				iAttributeSet = this
						.createAttributeSet((IAttributeDescription[]) tmpDescriptions
								.toArray());
				iAttributeSet.setName(resultSet.getString("ATTRIBUTESET_NAME"));
				iAttributeSet.setInfo(resultSet.getString("ATTRIBUTESET_INFO"));
	...
	public void remove(IAttributeSet attributeSet) throws PlatformException {
		...
		delete.addCondition("ATTRIBUTESET_ID =", attributeSet
				.getAttributeSetId());
		int rows;
		try {
